...
<div id="page-wrapper">  
    <div class="row">
                    <div class="col-lg-12">
                        <h1 class="page-header">Demand Letter>>Edit</h1>
                    </div>
                    <!-- /.col-lg-12 -->
                </div>
    <div class="row">
                            ...
                            <div class="panel-body">
                                <div class="dataTable_wrapper">
                                    <div id="dataTables-example_wrapper" class="dataTables_wrapper form-inline dt-bootstrap no-footer">
                                        ...
                                        <tbody>
                                           {!! Form::model($deman, ['method' => 'PATCH','route' => ['demand.update', $deman->id]]) !!}
                                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                           <tr>
                                        <td width="20%">Title:</td><td> <input class="form-control" type="text" size="68" name="title" id="title" value="{{$deman->title}}">@if ($errors->has('title')) <p class="help-block">{{ $errors->first('title') }}</p> @endif</td>
                                                                                    
                                        </tr>
                                        <tr>
                                        <td width="20%">No of Vacancies:</td><td><input class="form-control" type="text" size="68" name="no_vacancy" id="no_vacancy" value="{{$deman->no_vacancy}}">@if ($errors->has('no_vacancy')) <p class="help-block">{{ $errors->first('no_vacancy') }}</p> @endif</td>
                                        </tr>
                                        <tr>
                                        <td width="20%">Minimum Qualification:</td><td><input class="form-control" type="text" size="68" name="qualification" id="qualification" value="{{$deman->qualification}}">@if ($errors->has('qualification')) <p class="help-block">{{ $errors->first('qualification') }}</p> @endif</td>
                                        </tr>
                                        <tr>
                                        <td width="20%">Location:</td><td><input class="form-control" type="text" size="68" name="location" id="location" value="{{$deman->location}}">@if ($errors->has('location')) <p class="help-block">{{ $errors->first('location') }}</p> @endif</td>
                                        </tr>
                                        <tr>
                                        <td width="20%">Description:</td><td> <textarea class="form-control" id="description" name="description" index="1" cols="70" rows="10">{{$deman->description}}</textarea></td>
                                        </tr>
